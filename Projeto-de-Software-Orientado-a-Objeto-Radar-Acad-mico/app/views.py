...
from .forms import TurmaForm, CursoForm, PessoaForm, InstituicaoForm, DisciplinaForm, CidadeForm, MatriculaForm, FrequenciaForm
from .models import (
...
def curso_editar(request, id):
    ...
    form = CursoForm(request.POST or None, instance=obj)
    if form.is_valid(): form.save(); return redirect('curso_list')
...
def disciplina_editar(request, id):
    ...
    form = DisciplinaForm(request.POST or None, instance=obj)
    if form.is_valid(): form.save(); return redirect('disciplina_list')
...
def cidade_editar(request, id):
    ...
    form = CidadeForm(request.POST or None, instance=obj)
    if form.is_valid(): form.save(); return redirect('cidade_list')
...
def matricula_list(request):
    return lista_generica(request, Matricula, 'app/matricula_list.html', 'matriculas')
...
def matricula_editar(request, id):
    ...
    form = MatriculaForm(request.POST or None, instance=obj)
    if form.is_valid(): form.save(); return redirect('matricula_list')
